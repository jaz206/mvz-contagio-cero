...
const GameContent: React.FC = () => {
    ...
    const {
        user, loading, loadingAuth, lang, viewMode, playerAlignment, heroes, completedMissionIds,
        omegaCylinders, worldStage, activeGlobalEvent, surferTurnCount, tickerMessage,
        isEditorMode, showMissionEditor, missionToEdit, showCharacterEditor, showDbManager,
        selectedMission, showStory, ownedExpansions, showExpansionConfig,
        visibleMissions, introMission, groupedMissions, expandedZones, isSidebarCollapsed,
        isSaving, FACTION_STATES, getAllMissions // Wait, getAllMissions not exposed?
    } = state;

    // Need to expose allMissions from hook if used
    const { allMissions } = state;
    ...
    return (
        <div className={`flex flex-col h-screen w-full bg-slate-950 text-cyan-400 font-sans overflow-hidden relative ${playerAlignment === 'ZOMBIE' ? 'animate-pulse-slow grayscale-[0.3] sepia-[0.2] contrast-125' : ''}`}>
            <CharacterEditor isOpen={showCharacterEditor} onClose={() => actions.setShowCharacterEditor(false)} language={lang} />
            <MissionEditor
            ...
            {activeGlobalEvent && <EventModal event={activeGlobalEvent} isOpen={!!activeGlobalEvent} onAcknowledge={actions.handleEventAcknowledge} language={lang} playerAlignment={playerAlignment} />}

            {selectedMission && <MissionModal
            ...
            <DatabaseManager isOpen={showDbManager} onClose={() => actions.setShowDbManager(false)} language={lang} />
            ...
            {viewMode === 'login' && (<LoginScreen onLogin={actions.handleGuestLogin} onGoogleLogin={() => { }} onEditorLogin={actions.handleEditorLogin} language={lang} setLanguage={actions.setLang} />)}

            {viewMode === 'story' && (<StoryMode language={lang} onComplete={(choice) => { actions.setPlayerAlignment(choice); actions.setViewMode('setup'); }} onSkip={() => { actions.setPlayerAlignment('ALIVE'); const core = GAME_EXPANSIONS.find(e => e.id === 'core_box'); if (core) actions.setHeroes(core.heroes); actions.setViewMode('map'); }} startAtChoice={state.startStoryAtChoice} />)}

            {viewMode === 'setup' && playerAlignment && (<ExpansionSelector language={lang} playerAlignment={playerAlignment} onConfirm={actions.handleExpansionConfirm} onBack={() => { actions.setPlayerAlignment(null); actions.setStartStoryAtChoice(true); actions.setViewMode('story'); }} ownedExpansions={ownedExpansions} onToggleExpansion={actions.toggleExpansion} onToggleAllExpansions={actions.toggleAllExpansions} />)}
            ...
            {viewMode === 'intro' && playerAlignment && (
                <IntroSequence
                    ...
                    onComplete={() => {
                        if (introMission) {
                            actions.setViewMode('mission0');
                        } else {
                            actions.setViewMode('tutorial');
                        }
                    }}
                />
            )}
            ...
            {viewMode === 'mission0' && introMission ? (
                <MissionModal mission={introMission} isOpen={true} onClose={() => actions.setViewMode('tutorial')} onComplete={() => { actions.handleMissionComplete(introMission.id); actions.setViewMode('tutorial'); }} language={lang} isCompleted={false} />
            ) : viewMode === 'mission0' && !introMission ? (
                <div className="absolute inset-0 flex items-center justify-center bg-slate-950 text-red-500 font-bold z-50 flex-col gap-4">
                    ...
                    <button onClick={() => actions.setViewMode('map')} className="border border-red-500 px-4 py-2 hover:bg-red-900/20">SALTAR AL MAPA</button>
                </div>
            ) : null}
            ...
            {(viewMode === 'map' || viewMode === 'bunker' || viewMode === 'tutorial') && (
                <>
                    ...
                    <div className="flex-1 flex overflow-hidden relative">
                        <aside className={`flex-none glass-panel border-r border-cyan-900/50 flex flex-col z-20 shadow-2xl overflow-hidden relative transition-all duration-300 ${isSidebarCollapsed ? 'w-12' : 'w-80'}`}>
                            <button onClick={() => actions.setIsSidebarCollapsed(!isSidebarCollapsed)} className="absolute top-1/2 -right-3 w-6 h-12 bg-cyan-900 border border-cyan-600 rounded-l flex items-center justify-center z-50 hover:bg-cyan-800 transition-colors" style={{ transform: 'translateY(-50%)' }}><span className="text-xs text-cyan-200">{isSidebarCollapsed ? '›' : '‹'}</span></button>
                            {!isSidebarCollapsed ? (
                                <>
                        ...
                        <main className="flex-1 relative bg-slate-950 overflow-hidden">
                            {viewMode === 'map' && (
                                <>
                                    <USAMap language={lang} missions={visibleMissions} completedMissionIds={completedMissionIds} onMissionComplete={actions.handleMissionComplete} onMissionSelect={actions.handleMissionSelect} onBunkerClick={() => actions.setViewMode('bunker')} factionStates={FACTION_STATES} playerAlignment={playerAlignment} worldStage={worldStage} surferTurnCount={surferTurnCount} />
