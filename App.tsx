    ...
    return (
        <div className="flex flex-col h-screen w-full font-sans overflow-hidden relative transition-colors duration-500">
            ...
            <AdminStaffPanel
                ...
                onSetOmegaCylinders={(value) => void actions.persistOmegaCylinders(Math.max(0, Math.min(15, value)))}
            />
...
const GameContent: React.FC = () => {
    ...
    return (
        <Routes>
            ...
            <Route path="/story" element={<StoryMode language={lang} slides={state.storyConfig.slides} onComplete={actions.handleStoryChoice} onSkip={() => { actions.setStartStoryAtChoice(true); void actions.syncCampaignMeta(playerAlignment || null, 'story'); navigate('/story', { replace: true }); }} startAtChoice={state.startStoryAtChoice} />} />

            <Route path="/setup" element={effectiveAlignment ? (shouldSkipSetup ? <Navigate to={setupExitRoute} replace /> : <ExpansionSelector language={lang} playerAlignment={effectiveAlignment} onConfirm={actions.handleExpansionConfirm} onBack={() => { actions.setPlayerAlignment(null); actions.setStartStoryAtChoice(true); void actions.syncCampaignMeta(null, 'story'); navigate('/story'); }} ownedExpansions={state.ownedExpansions} onToggleExpansion={actions.toggleExpansion} onToggleAllExpansions={actions.toggleAllExpansions} />) : <Navigate to="/" />} />

            <Route path="/intro" element={effectiveAlignment ? <IntroSequence language={lang} playerAlignment={effectiveAlignment} slides={effectiveAlignment === 'ZOMBIE' ? state.introConfig.zombie : state.introConfig.alive} onComplete={() => { actions.setIsStartingCampaign(false); if (state.introMission && !hasCompletedIntroMission) { void actions.syncCampaignMeta(effectiveAlignment, 'mission0'); navigate('/mission0'); } else if (!hasSeenTutorial) { void actions.syncCampaignMeta(effectiveAlignment, 'tutorial'); navigate('/tutorial'); } else { void actions.syncCampaignMeta(effectiveAlignment, 'map'); navigate('/map'); } }} /> : <Navigate to="/" />} />

            <Route path="/mission0" element={(effectiveAlignment && state.introMission) ? <MissionModal mission={state.introMission} isOpen={true} onClose={() => { void actions.syncCampaignMeta(effectiveAlignment, 'map'); navigate('/map'); }} onComplete={async (_missionId, reward) => { await actions.handleMissionComplete(state.introMission!.id, reward); if (reward?.keepModalOpen) return; void actions.syncCampaignMeta(effectiveAlignment, hasSeenTutorial ? 'map' : 'tutorial'); navigate(hasSeenTutorial ? '/map' : '/tutorial'); }} language={lang} isCompleted={hasCompletedIntroMission} /> : <Navigate to="/map" />} />
            ...
            <Route path="/bunker" element={<GameLayout publishedPreviewMode={isPublishedPreviewActive} sidebarGroupedMissions={sidebarGroupedMissions}><BunkerInterior heroes={heroes} missions={bunkerMissions} completedMissions={bunkerCompletedMissions} completedMissionIds={completedMissionIds} zoneControlConfig={state.zoneControlConfig} onAssign={(heroId, missionId) => { const heroIndex = heroes.findIndex((hero) => hero.id === heroId); if (heroIndex >= 0) { const nextHeroes = [...heroes]; nextHeroes[heroIndex] = { ...nextHeroes[heroIndex], status: 'DEPLOYED', assignedMissionId: missionId }; void actions.persistHeroes(nextHeroes); return true; } return false; }} onUnassign={(heroId) => { const heroIndex = heroes.findIndex((hero) => hero.id === heroId); if (heroIndex >= 0) { const nextHeroes = [...heroes]; nextHeroes[heroIndex] = { ...nextHeroes[heroIndex], status: 'AVAILABLE', assignedMissionId: null }; void actions.persistHeroes(nextHeroes); } }} onAddHero={(hero) => { void actions.persistHeroes([...heroes, hero]); }} onToggleObjective={actions.handleToggleHeroObjective} onBack={() => navigate(currentFlowStep === 'tutorial' ? '/tutorial' : '/map')} language={lang} playerAlignment={playerAlignment} isEditorMode={!isPublishedPreviewActive && isEditorMode} onTransformHero={actions.handleTransformHero} onTickerUpdate={actions.handleTickerUpdate} omegaCylinders={omegaCylinders} onSearchAllies={() => void actions.persistOmegaCylinders(Math.max(0, omegaCylinders - 1))} ownedExpansions={state.ownedExpansions} /></GameLayout>} />

            <Route path="/tutorial" element={<div className="absolute inset-0 z-40"><USAMap language={lang} missions={state.visibleMissions} completedMissionIds={completedMissionIds} onMissionComplete={() => { }} onMissionSelect={() => { }} onBunkerClick={() => navigate('/bunker')} factionStates={state.FACTION_STATES} playerAlignment={playerAlignment} worldStage={worldStage} controlledZones={state.controlledZones} /><TutorialOverlay language={lang} onComplete={() => { localStorage.setItem(tutorialKey, 'true'); void actions.syncCampaignMeta(effectiveAlignment || 'ALIVE', 'map'); navigate('/map'); }} onStepChange={(stepKey) => { if (['roster', 'file', 'recruit'].includes(stepKey)) { navigate('/bunker'); } }} /></div>} />
