...
const HeroToken: React.FC<{ hero: Hero; onClick: () => void }> = ({ hero, onClick }) => {
    ...
    useEffect(() => {
        ...
        switch (hero.status) {
            case 'AVAILABLE': minX = 10; maxX = 25; minY = 20; maxY = 80; break;
            case 'DEPLOYED': minX = 75; maxX = 90; minY = 10; maxY = 40; break;
            case 'INJURED': minX = 75; maxX = 90; minY = 60; maxY = 90; break;
            default: return; 
        }
        ...
        const timer = setTimeout(moveToken, 100);
        const interval = setInterval(moveToken, 4000);
    ...
    // Fallback colors inline to avoid PurgeCSS issues
    const borderColor = hero.status === 'INJURED' ? '#ef4444' : hero.status === 'DEPLOYED' ? '#eab308' : '#06b6d4';

    return (
        <div 
            onClick={(e) => { e.stopPropagation(); onClick(); }}
            className="absolute rounded-full border-2 cursor-pointer hover:scale-110 transition-transform duration-300 flex items-center justify-center pointer-events-auto shadow-lg"
            style={{ 
                width: '48px', 
                height: '48px',
                zIndex: 50,
        ...
        >
            ...
            <span 
                className="text-[10px] font-bold text-white pointer-events-none absolute"
                style={{ opacity: hero.imageUrl ? 0 : 1 }}
            >
  ...
  const handleRecruitSubmit = async (e: React.FormEvent) => {
      ...
      if (isEditingExisting && recruitForm.templateId) {
          const updateData: Partial<HeroTemplate> = {
              defaultName: recruitForm.name, alias: recruitForm.alias, defaultClass: recruitForm.class, bio: recruitForm.bio, currentStory: recruitForm.currentStory, objectives: recruitForm.objectives, imageUrl: recruitForm.imageUrl, defaultStats: { strength: recruitForm.str, agility: recruitForm.agi, intellect: recruitForm.int }
          };
          await updateHeroTemplate(recruitForm.templateId, updateData);
          alert("DATABASE UPDATED.");
      } else {
          const newHero: Hero = {
              id: `custom_${Date.now()}`, templateId: recruitForm.templateId || undefined, name: recruitForm.name || 'UNKNOWN', alias: recruitForm.alias || 'AGENT', class: recruitForm.class, bio: recruitForm.bio, currentStory: recruitForm.currentStory, objectives: recruitForm.objectives, completedObjectiveIndices: [], imageUrl: recruitForm.imageUrl, status: 'AVAILABLE', stats: { strength: recruitForm.str, agility: recruitForm.agi, intellect: recruitForm.int }, assignedMissionId: null
          };
          onAddHero(newHero);
      }
    ...
    <div className="w-full h-full bg-slate-950 font-mono relative overflow-hidden select-none">
        ...
        <div className="absolute inset-0 z-0 pointer-events-auto" style={{ width: '100%', height: '100%' }}>
            <svg width="100%" height="100%" viewBox="0 0 1000 600" preserveAspectRatio="xMidYMid slice" className="opacity-40">
                ...
                <text x="175" y="530" textAnchor="middle" fill="#06b6d4" fontSize="14" fontWeight="bold">BARRACKS</text>
                <path d="M 350 50 L 650 50 L 650 300 L 350 300 Z" fill="none" stroke="#06b6d4" strokeWidth="2" />
                <text x="500" y="320" textAnchor="middle" fill="#06b6d4" fontSize="14" fontWeight="bold">COMMAND</text>
                <rect x="380" y="70" width="240" height="120" fill="#0f172a" stroke="#06b6d4" strokeWidth="2" onClick={onBack} className="cursor-pointer hover:stroke-white" />
                <path d="M 700 50 L 950 50 L 950 300 L 700 300 Z" fill="none" stroke="#eab308" strokeWidth="2" />
                <text x="825" y="320" textAnchor="middle" fill="#eab308" fontSize="14" fontWeight="bold">HANGAR</text>
                <path d="M 700 350 L 950 350 L 950 550 L 700 550 Z" fill="none" stroke="#ef4444" strokeWidth="2" />
                <text x="825" y="570" textAnchor="middle" fill="#ef4444" fontSize="14" fontWeight="bold">MEDBAY</text>
                <g onClick={() => setShowRecruitModal(true)} className="cursor-pointer"><rect x="450" y="450" width="100" height="60" fill="#0f172a" stroke="#06b6d4" strokeWidth="2" /><text x="500" y="485" textAnchor="middle" fill="#06b6d4" fontSize="10" fontWeight="bold">DATABASE</text></g>
            </svg>
        </div>

        {/* HERO TOKENS LAYER - z-50 - EXPLICIT PLACEMENT */}
        <div 
        ...
        {selectedHeroId && selectedHero && (
            <div className="absolute inset-0 z-[60] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 md:p-12 animate-fade-in" onClick={() => setSelectedHeroId(null)}>
                <div className="w-full max-w-4xl bg-slate-900 border-2 border-cyan-500 shadow-2xl flex flex-col max-h-full overflow-hidden" onClick={e => e.stopPropagation()}>
                    <div className="p-4 border-b border-cyan-800 bg-cyan-900/20 flex justify-between items-center">
                        ...
                        <button onClick={() => setSelectedHeroId(null)} className="text-cyan-500 hover:text-white">✕</button>
                    </div>
                    
                    <div className="flex-1 p-6 overflow-y-auto">
                        <div className="flex flex-col md:flex-row gap-8">
                            <div className="w-full md:w-64 flex flex-col gap-4 shrink-0">
                                <div className="aspect-[3/4] bg-slate-950 border-2 border-cyan-700 relative group overflow-hidden shadow-inner cursor-zoom-in" onClick={() => setZoomedImage(selectedHero.imageUrl || '')}>
                                ...
                                {isEditorMode && selectedHero.templateId && (
                                    <button onClick={handleEditClick} className="w-full py-2 bg-blue-900/50 hover:bg-blue-800 text-blue-300 text-xs font-bold border border-blue-700 uppercase tracking-wider">
                                        EDIT DATA (DB)
                                    </button>
                                )}
                            </div>
                            ...
                            <div className="flex-1 flex flex-col gap-6">
                                <div className="flex justify-between items-end border-b border-cyan-800 pb-2">
                                    <div>
                                        ...
                                        <div className="text-xl text-white">{selectedHero.name}</div>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-xs text-cyan-600 font-bold uppercase">{t.bunker.status}</div>
                                        <div className={`text-lg font-bold ${selectedHero.status === 'AVAILABLE' ? 'text-emerald-400' : selectedHero.status === 'INJURED' ? 'text-red-500' : 'text-yellow-400'}`}>{selectedHero.status}</div>
                                    </div>
                                </div>

                                <div className="bg-slate-950/50 p-4 border-l-2 border-cyan-700">
                    ...
                    <div className="p-4 border-t border-cyan-800 bg-cyan-900/20 flex justify-between items-center">
                        ...
                        <div>
                            {selectedHero.status === 'DEPLOYED' ? (
                                <button onClick={() => onUnassign(selectedHero.id)} className="px-6 py-2 bg-red-900/50 border border-red-500 text-red-300 font-bold text-xs tracking-widest hover:bg-red-800">{t.bunker.unassign}</button>
                            ) : (
                                <button 
                                    disabled={selectedHero.status === 'INJURED'} 
                                    onClick={() => { setAssignError(null); setShowAssignModal(true); }} 
                                    className={`px-6 py-2 font-bold text-xs tracking-widest border transition-all ${selectedHero.status === 'INJURED' ? 'bg-gray-800 border-gray-600 text-gray-500 cursor-not-allowed' : 'bg-cyan-700 border-cyan-500 text-white hover:bg-cyan-600 shadow-[0_0_15px_rgba(6,182,212,0.4)]'}`}
                                >
                                    {t.bunker.assign}
                                </button>
                            )}
