...
export const IntroSequence: React.FC<IntroSequenceProps> = ({ language, onComplete, playerAlignment }) => {
    ...
    // SELECCIONAR LOS SLIDES CORRECTOS SEGÚN EL BANDO
        ...
        <div className="fixed inset-0 z-[100] bg-black flex flex-col items-center justify-center overflow-hidden font-mono">
            ...
            <div className={`relative z-20 w-full max-w-4xl p-8 mt-auto mb-12 transition-all duration-1000 transform ${fadeIn ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                ...
                <div className="flex items-center gap-4 mb-4">
                    <div className={`h-px flex-1 ${playerAlignment === 'ZOMBIE' ? 'bg-lime-900' : 'bg-cyan-900'}`}></div>
                    <span className={`${playerAlignment === 'ZOMBIE' ? 'text-lime-500' : 'text-cyan-500'} text-xs font-bold tracking-[0.3em] uppercase`}>
                        ARCHIVO: {playerAlignment === 'ZOMBIE' ? 'HUNGER_LOG' : 'MH0'} // SECUENCIA {currentIndex + 1} DE {slides.length}
                    </span>
                    <div className={`h-px flex-1 ${playerAlignment === 'ZOMBIE' ? 'bg-lime-900' : 'bg-cyan-900'}`}></div>
                </div>
                ...
                <div className="mt-8 flex justify-center">
                        ...
                        className={`group relative px-8 py-3 border font-bold tracking-widest transition-all duration-300
                            ${isAnimating 
                                ...
                                : playerAlignment === 'ZOMBIE'
                                    ? 'bg-lime-900/30 border-lime-500 text-lime-400 hover:bg-lime-500 hover:text-black'
                    ...
                    >
                        ...
                        {!isAnimating && (
                            <div className={`absolute inset-0 ${playerAlignment === 'ZOMBIE' ? 'bg-lime-500' : 'bg-cyan-500'} transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-300 -z-0`}></div>
                        )}
                    </button>
                </div>
            </div>
