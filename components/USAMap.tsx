...
export const USAMap: React.FC<USAMapProps> = ({
    ...
    useEffect(() => {
        ...
        validMissions.forEach(m => {
            const prereqs = m.prereqs || (m.prereq ? [m.prereq] : []);
            prereqs.forEach(pid => connections.push({ source: pid, target: m.id }));
        });
        ...
        <div ref={containerRef} className="w-full h-full relative bg-transparent overflow-hidden group">
            {worldStage === 'GALACTUS' && (
                <div className="absolute inset-0 z-30 pointer-events-none overflow-hidden flex items-center justify-center">
                    ...
                    <div className="w-[120%] h-[120%] absolute animate-breathing opacity-60 flex items-center justify-center" style={{ mixBlendMode: 'hard-light', maskImage: 'radial-gradient(circle, black 40%, transparent 80%)', WebkitMaskImage: 'radial-gradient(circle, black 40%, transparent 80%)' }}>
                        <img
                            src="https://i.pinimg.com/736x/7d/99/05/7d9905d417620745edc2c724f94e56c8.jpg"
            ...
            {tooltip && (
                <div className="fixed z-50 pointer-events-none glass-panel p-4 shadow-2xl rounded-sm min-w-[240px] font-sans animate-fade-in border-cyan-500/50" style={{ left: Math.min(tooltip.x + 15, window.innerWidth - 260), top: Math.min(tooltip.y + 15, window.innerHeight - 150) }}>
                    <div className="flex justify-between items-start mb-2 border-b border-cyan-500/20 pb-2">
                        <div className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.2em]">{tooltip.factionName}</div>
                        <div className={`text-[9px] font-black px-2 py-0.5 rounded-full border ${tooltip.mission.threatLevel === 'EXTREMA' ? 'bg-red-950/40 text-red-500 border-red-500/50' : 'bg-orange-950/40 text-orange-500 border-orange-500/50'}`}>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="text-[10px] text-slate-400 font-medium">CLICK TO VIEW MISSION DETAILS</div>
                    </div>
                </div>
            )}
        </div>
