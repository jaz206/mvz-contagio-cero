...
export const ExpansionSelector: React.FC<ExpansionSelectorProps> = ({ 
    ...
    return (
        <div className="fixed inset-0 z-[80] bg-slate-950 flex flex-col font-mono overflow-hidden">
            ...
            {/* MAIN CONTENT */}
            <div className="flex-1 overflow-y-auto p-4 md:p-8 pb-36 scrollbar-thin scrollbar-thumb-slate-600 relative z-10">
                                        ...
                                        {exp.heroesToShow.map(hero => {
                                            ...
                                            return (
                                                <div 
                                                    ...
                                                    className={`
                                                        relative group cursor-pointer border-2 transition-all duration-200 overflow-hidden rounded-sm
                                                        ${isSelected 
                                                            ? `${borderColor} bg-slate-800 ${glowClass} transform scale-105 z-10` 
                                                            : isDisabled 
                                                                ? 'border-slate-700 opacity-30 grayscale cursor-not-allowed' 
                                                                : 'border-slate-600 bg-slate-900 hover:border-white hover:bg-slate-800'}
                                                    `}
                                                >
                                                    {isSelected && (
                                                        <div className={`absolute top-0 right-0 p-1.5 z-20 ${bgColor} shadow-lg border-l border-b border-black`}>
                                                            <svg className="w-4 h-4 text-black font-bold" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="4"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"></path></svg>
                                                        </div>
                                                    )}
            ...
            {/* FOOTER */}
            <div className={`h-28 border-t-4 ${borderColor} bg-slate-900 absolute bottom-0 left-0 right-0 z-50 flex items-center px-4 md:px-8 justify-between shadow-[0_-10px_50px_rgba(0,0,0,1)]`}>
                <div className="flex items-center gap-3 overflow-x-auto flex-1 mr-4 scrollbar-hide py-2">
                    <div className="flex flex-col justify-center mr-4 shrink-0 text-center">
                        <span className={`text-4xl font-black ${selectedHeroes.length === 6 ? 'text-red-500 animate-pulse' : textColor}`}>
                    ...
                    {[...Array(6)].map((_, i) => {
                        ...
                        return (
                            <div 
                                ...
                                className={`
                                    w-16 h-20 border-2 shrink-0 relative flex items-center justify-center transition-all group
                                    ${hero 
                                        ? `${borderColor} cursor-pointer bg-slate-800` 
                                        : 'border-slate-700 bg-black border-dashed'}
                                `}
                            >
                                {hero ? (
                                    <>
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 bg-black text-[8px] text-white text-center py-0.5 truncate px-1 font-bold">
                                            {hero.alias}
                                        </div>
                                    </>
                                ) : (
                ...
                <button 
                    ...
                    className={`
                        px-8 py-4 font-black tracking-[0.2em] text-sm transition-all shadow-lg shrink-0 border-2
                        ${selectedHeroes.length > 0
                            ? isZombie 
                                ? 'bg-lime-600 hover:bg-lime-500 text-black border-lime-400 hover:shadow-[0_0_20px_rgba(132,204,22,0.8)]' 
                                : 'bg-cyan-600 hover:bg-cyan-500 text-black border-cyan-400 hover:shadow-[0_0_20px_rgba(6,182,212,0.8)]'
